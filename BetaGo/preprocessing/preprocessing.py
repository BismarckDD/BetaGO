...
""" A feature encoding the number of liberties of the group connected to the stone at each location
Note:
...
class Preprocess(object):
    ...
    """ Create a preprocessor object that will concatenate
        together the given list of features """
    ...
    def __init__(self, feature_list=DEFAULT_FEATURES):
        ...
        for i in range(len(feature_list)):
            feature = feature_list[i].lower()
            if feature in FEATURES:
                self.processors[i] = FEATURES[feature]["function"]
