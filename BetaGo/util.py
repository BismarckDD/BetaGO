...
def flatten_index(position, size):
    (x, y) = position
    return x * size + y


def unflatten_index(idx, size):
    x, y = divmod(idx, size)
    return (x, y)
...
""" Iterates over (GameState, move, player) tuples in the first game of the given SGF file.
    Ignores variations - only the main line is returned.  The state object is modified in-place,
    so don't try to, for example, keep track of it through time
...
def save_gamestate_to_sgf(gamestate, path, filename,
